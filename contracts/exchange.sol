...
contract Mergex is EtherStore{
	...
	event Trade(uint timestamp, address tokenA, address tokenB, uint valueA, uint valueB);
	...
	function getFillValue(address owner, bytes32 hash) public returns (uint filled){
	...
	function fillOrder(address owner, address tokenA, address tokenB, uint tradeAmount, uint valueA, uint valueB, uint expiration, uint nonce, uint8 v, bytes32 r, bytes32 s) public{
		...
		if(validateOrder(owner,hash,expiration,tradeAmount,valueA,v,r,s)){
			...
			Trade(now, tokenA, tokenB, tradeAmount, valueB.mul(tradeAmount).div(valueA));
	...
	function tradeTokens(address userA,address userB,address tokenA,address tokenB,uint amountA,uint valueA,uint valueB) internal returns(bool success){
